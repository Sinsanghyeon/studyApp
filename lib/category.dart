    ...
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: const [
          ...
          BottomNavigationBarItem(icon: Icon(Icons.video_library), label: '홈'),
          BottomNavigationBarItem(icon: Icon(Icons.video_library), label: '채팅'),
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            Row(children: [
              Expanded(child: Text('일시: ${DateFormat('yyyy-MM-dd').format(selectedDate)}')),
              TextButton(
                onPressed: () async {
            ...
            DropdownButtonFormField<String>(
              decoration: const InputDecoration(labelText: '모임장소'),
              value: locationMode,
              items: const [
                DropdownMenuItem(value: '온라인', child: Text('온라인')),
                DropdownMenuItem(value: '오프라인', child: Text('오프라인')),
              ],
              onChanged: (val) => locationMode = val!,
            ),
            const SizedBox(height: 12),
            Row(children: [
              Expanded(child: TextField(controller: minCtrl, decoration: const InputDecoration(labelText: '최소인원'), keyboardType: TextInputType.number)),
            ...
            ElevatedButton(
              onPressed: () {
                setState(() {
                  final rawTags = tagsCtrl.text.split(',').map((e) => e.trim()).where((e) => e.isNotEmpty).toList();
                  studies.add({
                    'type': locationMode,
                    'typeColor': locationMode == '온라인' ? Colors.teal : Colors.orangeAccent,
                    'title': titleCtrl.text,
                    'desc': descCtrl.text,
                    'tags': rawTags,
                    'date': DateFormat('yyyy-MM-dd').format(selectedDate),
                    'location': locationMode,
                    'minMembers': int.tryParse(minCtrl.text) ?? 1,
                    'maxMembers': int.tryParse(maxCtrl.text) ?? 1,
...
class ContentPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Center(child: Text('홈'));
}
